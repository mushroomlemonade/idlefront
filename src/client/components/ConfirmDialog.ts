...
export class ConfirmDialog extends LitElement {
  ...
  @property({ type: Boolean }) commandFlyout = false;
  ...
  private renderOverlay() {
    ...
    return html`
      <div
        class="fixed inset-0 z-[10020] flex items-center justify-center bg-black/80 ${this.commandFlyout ? "atlas-command-confirm" : ""}"
        @click=${(e: Event) => {
          if (e.target === e.currentTarget) this.handleCancel();
        }}
      >
        <div
          class="relative mx-4 w-full ${
            this.wide ? "max-w-md" : "max-w-sm"
          } p-6 rounded-2xl border ${borderColor} ${cardBg} shadow-2xl"
        >
          ${
            this.showClose
              ? html`<button
